...
export default function ALAINLogo({
  ...
  return (
    <div className={`inline-flex items-center space-x-2 ${className}`}>
      <div className={`${sizeClasses[size]} ${colorClasses[variant]} relative`} aria-hidden>
        <svg viewBox="0 0 32 32" fill="none" className="h-full w-full" xmlns="http://www.w3.org/2000/svg">
          <rect x="4" y="6" width="20" height="20" rx="2" fill="currentColor" fillOpacity="0.1" stroke="currentColor" strokeWidth="1.5" />
          <line x1="8" y1="12" x2="20" y2="12" stroke="currentColor" strokeWidth="0.8" strokeOpacity="0.6" />
          <line x1="8" y1="16" x2="20" y2="16" stroke="currentColor" strokeWidth="0.8" strokeOpacity="0.6" />
          <line x1="8" y1="20" x2="16" y2="20" stroke="currentColor" strokeWidth="0.8" strokeOpacity="0.6" />
          <circle cx="24" cy="8" r="3" fill="var(--brand-spark)" />
      ...
      <span className={`font-semibold ${variant === 'light' ? 'text-white' : variant === 'dark' ? 'text-slate-900' : 'text-slate-900'}`}>
        ALAIN
      </span>
    </div>
  );
